...
RUN npm run build -w packages/buildings -w packages/character -w packages/market
...
ARG NEXT_PUBLIC_APPWRITE_FCM_PROVIDER_ID
...
ENV NEXT_PUBLIC_APPWRITE_FCM_PROVIDER_ID=$NEXT_PUBLIC_APPWRITE_FCM_PROVIDER_ID
